<template>
  <div class="getlist">
    <el-row>
      ...
      <el-col :span="5" class="cols btncol">
        <el-button type="primary" @click="cooperation = true">新增</el-button>
    ...
    >
      ...
      <el-table-column prop="state" label="是否展示">
        <template #default="props">
          <template v-if="props.row.state == '1'">
            <span class="next">展示</span>
          </template>
          <template v-else>
            <span class="clos">隐藏</span>
          </template>
        </template>
      </el-table-column>
    ...
    />
    <!-- 添加公告弹窗 -->
    <el-dialog v-model="cooperation" title="新增公告">
      <template #footer>
        <el-form
          ref="ruleFormRef"
          :model="addruleForm"
...
import apilogin from "@/api/setaddlist";
...
let submitForm = () => {
  // 拿到表单数据
  announcement(addruleForm.value);
};
// 提交操作
let announcement = (v) => {
  if (v.name != "" && v.text != "" && v.date != "") {
    loading.value = true;
    apilogin.announcement(v).then((res) => {
      if (res.data.code == 1) {
